...
$TARGET_REF = "refs/heads/main"         # Using the main branch ref
...
foreach ($analysis_id in $analysis_ids) {
    ...
    $sarifData = @{
        ...
        runs = @(
            @{
                tool = @{
                    driver = @{
                        name = "CodeQL"  # Update with actual tool name
                        informationUri = "https://codeql.github.com"  # Replace with correct tool information URL
                    }
                }
                results = @()
            }
    ...
    # Upload the SARIF content to the target repository using refs/heads/main
    ...
    $response = Invoke-RestMethod -Uri "https://api.github.com/repos/$TARGET_REPO/code-scanning/sarifs" `
        ...
        -Body (@{
            ...
            ref = "refs/heads/migration/$analysis_id"
            sarif = $gzipBase64
            tool_name = "CodeQL"  # Update with the actual tool name if needed
